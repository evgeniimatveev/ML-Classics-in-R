...
library(data.table)
setDT(data)  # Convert to data.table if it's not already

# Function to normalize numerical features (min-max scaling)
normalize <- function(x) {
  return((x - min(x)) / (max(x) - min(x)))  # Scale values between 0 and 1
}

# Identify numerical columns in the dataset
numeric_features <- names(data)[sapply(data, is.numeric)]

# Apply normalization using `:=` to update `data.table` in-place
data[, (numeric_features) := lapply(.SD, normalize), .SDcols = numeric_features]
...
performance_df <- data.frame(
  Model = c("K-Nearest Neighbors (KNN)", "Random Forest", "XGBoost"),  # Model names
  Accuracy = c(knn_cm$overall["Accuracy"], rf_cm$overall["Accuracy"], xgb_cm$overall["Accuracy"]),  # Accuracy scores
  Precision = c(knn_metrics["Precision"], rf_metrics["Precision"], xgb_metrics["Precision"]),  # Precision scores
  Recall = c(knn_metrics["Recall"], rf_metrics["Recall"], xgb_metrics["Recall"]),  # Recall scores
  F1_Score = c(knn_metrics["F1"], rf_metrics["F1"], xgb_metrics["F1"])  # F1-score values
)
...
performance_long <- melt(performance_df, id.vars = "Model")

# Remove missing values if any exist
performance_long <- na.omit(performance_long)

ggplot(performance_long, aes(x = Model, y = value, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge") +  # Grouped bars
  geom_text(aes(label = round(value, 3)),  # Round values to 3 decimal places
            position = position_dodge(width = 0.9), vjust = -0.5, size = 4) +  # Adjust label position
  labs(title = "Performance Comparison of ML Models",
       x = "Model",
       y = "Metric Value",
       fill = "Metric") +
  theme_minimal() +  # Clean theme
  scale_fill_manual(values = c("blue", "red", "green", "purple")) +  # Custom colors
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate labels for readability


summary_stats <- summary(data[, ..numeric_features])  # View Min, Max, Mean, etc.
print(summary_stats)  # Print to console


# Load the required library for table visualization
library(knitr)

# Print a formatted table with model performance metrics
kable(performance_df, caption = "🔍 Model Performance Comparison")
